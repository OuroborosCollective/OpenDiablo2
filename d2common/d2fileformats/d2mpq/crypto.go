...
func (c *crypter) cryptoLookup(index uint32) uint32 {
	return c.buffer[index]
}

//nolint:gomnd // Decryption magic
func (c *crypter) initialize() {
...
//nolint:gomnd // Decryption magic
func (c *crypter) decrypt(data []uint32, seed uint32) {
	seed2 := uint32(0xeeeeeeee)

	for i := 0; i < len(data); i++ {
		seed2 += c.cryptoLookup(0x400 + (seed & 0xff))
		result := data[i]
		result ^= seed + seed2
...
//nolint:gomnd // Decryption magic
func (c *crypter) decryptBytes(data []byte, seed uint32) {
	seed2 := uint32(0xEEEEEEEE)
...
//nolint:gomnd // Decryption magic
func (c *crypter) decryptTable(r io.Reader, size uint32, name string) ([]uint32, error) {
...
func (c *crypter) hashFilename(key string) uint64 {
	a, b := c.hashString(key, 1), c.hashString(key, 2)
	return uint64(a)<<32 | uint64(b)
}

//nolint:gomnd // Decryption magic
func (c *crypter) hashString(key string, hashType uint32) uint32 {
	seed1 := uint32(0x7FED7FED)
	seed2 := uint32(0xEEEEEEEE)

	/* prepare seeds. */
	for _, char := range strings.ToUpper(key) {
		seed1 = c.cryptoLookup((hashType*0x100)+uint32(char)) ^ (seed1 + seed2)
...
func (c *crypter) encrypt(data []uint32, seed uint32) {
	seed2 := uint32(0xeeeeeeee)
