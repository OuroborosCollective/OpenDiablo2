...
// GetWorldStatus returns resonance, cycle, expansion, and entropy.
func (e *BaalAalEngine) GetWorldStatus() (float64, float64, float64, float64) {
	e.EventBus.RLock()
	cycle := e.EventBus.ResonanceState
	resonance := math.Mod(cycle, 1.0)
	e.EventBus.RUnlock()

	e.WorldSystem.RLock()
	defer e.WorldSystem.RUnlock()

	return resonance, cycle, e.WorldSystem.Expansion, e.WorldSystem.Entropy
}
